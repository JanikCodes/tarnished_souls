            ...
            for item in self.items:
                ...
                if sell_item and not self.duplicates:
                    if sell_item.get_count() >= item.get_count():
                        ...
                        edited_embed.colour = discord.Color.red()
                        edited_embed.set_footer(text=f"You no longer have any {self.title.lower()} duplicates to sell..")
                        await interaction.message.edit(embed=edited_embed, view=None)
                        return
                    ...
                    match self.duplicates:
                        ...
                        case False:
                            edited_embed.colour = discord.Color.red()
                            if self.title == "Items":
                                edited_embed.set_footer(text=f"You no longer have any {self.title[:-1].lower()} to sell..")
                            else:
            ...
            match self.duplicates:
                ...
                case False:
                    match self.title:
                        ...
                        case "Items":
                            edited_embed.title = f"**All {self.title.lower()} have been sold!**"
            edited_embed.set_field_at(index=0, name="Amount of items:", value=f"{self.amount}\n\nYou received {self.value} runes!")
...
class SellAllView(discord.ui.View):
    ...
    def __init__(self, user, label, items, amount, value, duplicates):
        super().__init__()
        self.user = user.update_user()
        self.add_item(SellAllButton(user=self.user, label=label, items=items, amount=amount, value=value, duplicates=duplicates))
                ...
                if items:
                    ...
                    if duplicates:
                        ...
                        if len(items) != 0:
                            ...
                            new_embed = discord.Embed(title=f"You don't have any {choices.name} duplicates..", description="", colour=discord.Color.red())
